...
- Browse menu with categories, images, and multi-language support; consistent card layout with or without images; tap a dish photo to view it full screen; **"Recommended" section** highlights featured items at the top of the menu
...
- **New-order sound alert** — an audible chime when a new order arrives, configured per-device from the admin sidebar (the **Notification settings** button, reachable by every admin — each kitchen tablet/phone/computer has its own setting, stored in the browser). Choose from three sounds (service bell, marimba, doorbell), set volume, and "alert even when the device is muted" (so a silenced kitchen tablet still rings). Works on phones and tablets where OS notifications can't — the page itself plays the sound, so it reaches Android Chrome / iPad Safari while the dashboard is open. Per browser autoplay rules, staff tap once to enable sound after loading the dashboard. Sounds are royalty-free (CC0) and bundled in `public/sounds/`
- **Per-device admin text size** — each admin can set the console's text size (small / medium / large, default medium) from the sidebar; the choice is stored per browser/device, so a shared kitchen tablet can run larger text while a manager's laptop stays default
...
- **Takeaway / pickup orders** (optional — enable in Settings) — staff can place a takeaway order from the same **Place Order** screen: choose **Takeaway**, optionally add a customer name, and either leave the table blank (a counter pickup, with no table) or enter a table number (a seated guest who also wants something to go). Takeaway orders appear on the kitchen dashboard with a **Takeaway** badge and behave like any other order. A counter takeaway is settled directly from the board with **Mark collected**; a takeaway added to a seated table is settled together with that table at checkout. Reports break the day down by **Dine-in vs Takeaway** (order count, revenue, and each channel's share), and the order-history export carries an **Order type** column
