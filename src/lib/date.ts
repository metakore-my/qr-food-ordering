...
/**
 * Format a date to a sortable, locale-neutral `YYYY-MM-DD` in the given timezone.
 * This is the bookkeeping/pivot-friendly date key for exports: it sorts
 * lexicographically = chronologically, a spreadsheet/accountant reads it as a
 * real date (and Excel can filter "all of February" on it), and it carries no
 * day-vs-month ambiguity — unlike the human "5 Feb 2026" string. `en-CA` yields
 * `YYYY-MM-DD`. Single source of truth so every export emits the same shape.
 */
export function formatDeploymentDateKey(
  date: string | Date,
  timeZone = DEFAULT_TIMEZONE
): string {
...
/**
 * UTC `Date` for the END of `date`'s calendar day in `timeZone` — local
 * midnight of the NEXT day, used as an EXCLUSIVE upper bound (`createdAt < end`)
 * so a custom "to" date includes the whole of that day's orders. Built off
 * `startOfDayInZone` + 24h so it inherits the same DST-correct zone handling.
 */
