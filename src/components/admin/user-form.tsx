...
export function UserForm({ currentUserId }: { currentUserId: string }) {
  ...
  async function handleDeactivate(user: User) {
    if (!confirm(t("confirmDeactivate", { username: user.username }))) {
      return;
    }
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    // Client-side validation: min 8 chars, uppercase, lowercase, digit
    const errors: string[] = [];
              ...
              {String(user.id) === currentUserId || user.role === "SUPERADMIN" ? (
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {user.isActive ? (
                    <button
                      onClick={() => handleDeactivate(user)}
                      className="rounded-md border border-red-300 bg-red-50 px-3 py-1.5 text-xs font-medium text-red-700 transition-colors hover:bg-red-100"
                    >
                      {tCommon("deactivate")}
                    </button>
                  ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div role="dialog" aria-modal="true" aria-labelledby="user-form-title" className="mx-4 w-full max-w-md overflow-y-auto rounded-lg bg-white p-4 shadow-xl sm:p-6" style={{ maxHeight: "calc(100dvh - 2rem)" }}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button
                onClick={closeModal}
                className="flex h-10 w-10 items-center justify-center rounded-md text-gray-400 hover:text-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
