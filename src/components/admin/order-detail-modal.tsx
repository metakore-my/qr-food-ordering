  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.error === "Session is not active") {
          setError(tCheckout("alreadyCheckedOut"));
        } else if (data.error === "Session not found") {
          setError(tCheckout("invalidSession"));
        } else {
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-4 py-4 sm:px-6">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-md p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
            ...
            >
                ...
                {order.status === "PENDING" && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="flex min-h-[44px] items-center rounded-md bg-blue-600 px-3 py-2.5 text-xs font-medium text-white hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 disabled:opacity-50"
                    >
              ...
              <div className="space-y-1.5">
                {order.items.map((item) => {
                  ...
                  const opts: Array<{ groupName?: string; choiceName: string }> = item.selectedOptions
                    ...
                    >
                      ...
                      <div className="ml-2 flex items-center gap-1.5">
                        <button
                          ...
                          onClick={() => {
                            if (item.quantity === 1) {
                              if (!confirm(t("confirmRemoveItem", { name: getItemName(item.menuItem?.names ?? []) }))) {
                                return;
                              }
                            }
                            handleItemUpdate(
