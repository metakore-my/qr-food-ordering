...
export function CartItemRow({
  ...
  const name = getItemName(item.menuItem.names, locale, tCart("unknownItem"), cfg.defaultLocale);
  ...
  const basePrice = (item.menuItem.isCombo && item.menuItem.comboBasePrice != null)
    ...
    : item.menuItem.price;
  const lineTotal = (basePrice + optionAdj) * item.quantity;
    ...
    <div className={`rounded-lg bg-white p-3 shadow-sm${isUnavailable ? " opacity-50" : ""}`}>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {optionText && (
            <p className="line-clamp-2 text-xs text-gray-400">{optionText}</p>
          )}
          <p className="text-sm text-gray-500">
            {money(basePrice + optionAdj)}
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={() => {
            if (confirm(tCart("confirmRemoveItem"))) {
              onRemove(item.id);
            }
          }}
